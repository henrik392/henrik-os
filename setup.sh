...
FISHEOF
  ...
  # Oh My Fish + git plugin
  if [[ ! -d "$HOME/.local/share/omf" ]]; then
    info "Installing Oh My Fish..."
    fish -c 'curl -sL https://raw.githubusercontent.com/oh-my-fish/oh-my-fish/master/bin/install | fish /dev/stdin --noninteractive'
  else
    success "Oh My Fish already installed"
  fi

  fish -c 'omf install git' 2>/dev/null || true
